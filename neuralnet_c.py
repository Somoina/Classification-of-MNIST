...
class Neuralnetwork():
  ...
  def loss_func(self, logits, targets):
    ...
    '''
    self.targets=targets
    N=logits.shape[0];C=logits.shape[1]
    return -np.sum((np.dot(self.targets.T,np.log(logits))))/(N*C)
...
def trainer(model, X_train, y_train, X_valid, y_valid, config):
  ...
  #for epoch in range (15):  
      for i in range (int((X_train.shape[0])/(config['batch_size']))):
          ...
          lab=one_hot(lab, 10)
          (loss_train, y_tr)=model.forward_pass(X_train[(i*config['batch_size']):(i*config['batch_size'])+config['batch_size'],:],lab)
          model.backward_pass()
          loss_tr=loss_tr+loss_train
      y_train_hot=one_hot(y_train.astype(int), 10)
      y_valid_hot=one_hot(y_valid.astype(int), 10)
      (loss_training, y_training_logits) = model.forward_pass(X_train, y_train_hot)     
      (loss_valid,y_valid_logits)=model.forward_pass(X_valid, y_valid_hot)
      Loss_valid[:,epoch] = loss_valid
      Loss_train[:,epoch] = loss_tr/int((X_train.shape[0])/(config['batch_size']))
      #Get the accuracy
